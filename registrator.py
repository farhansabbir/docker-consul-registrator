...
def generate_Payload_For_Registration(container=None):
    ...
    if is_A_Service_Container(container=container):
        # syslog.syslog(syslog.LOG_INFO,"Service container: " + str(container.id))
        ...
        if len(svc["Endpoint"]["Ports"]) > 1:
            syslog.syslog(syslog.LOG_INFO,"WARNING! Multiple port bindings found. Using single exposed port only. This may impact service registration.")
        ...
        # syslog.syslog(syslog.LOG_INFO,"Regular container: " + str(container.id))
        ...
        for portproto,mapping in container.attrs["HostConfig"]["PortBindings"].items():
            if len(mapping) > 1:
                syslog.syslog(syslog.LOG_INFO,"Warning! Multiple ports exposed. Will select only first one: " + str(mapping[0]))
            IP = CONFIG["self_ip"]
            if mapping[0]["HostIp"] != "":
                IP = mapping[0]["HostIp"]
...
def deregister_Service_From_Consul(id=None):
    ...
    if resp.status_code == 200:
        syslog.syslog(syslog.LOG_INFO,"Successfully deregistered container " + id +  " from consul.")
    else:
        syslog.syslog(syslog.LOG_INFO,"Unable to deregister container " + id +  " from consul because " + str(resp.text))
...
def register_Service_To_Consul(container=None):
    ...
    if not data:
        syslog.syslog(syslog.LOG_INFO,"Container '" + str(container.name) + " (" + str(container.id) + ")' is not labelled to register with consul. Skipping.")
        return None
    headers = {"Content-type": "application/json"}
    resp = requests.put(CONFIG["consul"] + "/v1/agent/service/register",json=data, headers=headers)
    if resp.status_code == 200:
        syslog.syslog(syslog.LOG_INFO,"Successfully registered container (" + str(container.name) + " (" + str(container.id) + ")' to consul.")
    else:
        syslog.syslog(syslog.LOG_INFO,"Unable to register container '" + str(container.name) + " (" + str(container.id) + ")' to consul.")
        syslog.syslog(syslog.LOG_INFO,resp.text)
...
def cleanup():
    syslog.syslog(syslog.LOG_INFO,"Running cleanup.")
    syslog.syslog(syslog.LOG_INFO,"Validating with running containers first.")
    for container in DOCKER_CLIENT.containers.list():
        if not is_Container_Registered_To_Consul(container=container):
            register_Service_To_Consul(container)
        else:
            syslog.syslog(syslog.LOG_INFO,"Container '" + str(container.name) + " (" + str(container.id) + ")' is already registered to consul. Skipping.")
    syslog.syslog(syslog.LOG_INFO,"Cleaning up old stale/exited containers. Found total: " + str(len(get_Container_List_From_Host(status="exited"))))
    ...
    for svc_id in (get_Registered_Services_From_Consul()).keys():
        ...
        if (svc_id not in [container.id for container in get_Container_List_From_Host()]):
            syslog.syslog(syslog.LOG_INFO,"Deregistering non-existant container '" + svc_id + "' from consul")
            deregister_Service_From_Consul(svc_id)

    syslog.syslog(syslog.LOG_INFO,"Initial cleanup complete.")
